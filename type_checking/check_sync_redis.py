...
from reddish.backends._client_stubs import SyncRedis as Redis
from typing import assert_type


async def check_execute(redis: Redis):
    result = redis.execute(Command("...").into(int))
    assert_type(result, int)


async def check_execute_typeform(redis: Redis):
    result = redis.execute(Command("...").into(int | str))
